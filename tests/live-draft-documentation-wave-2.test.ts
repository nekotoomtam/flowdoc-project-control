...
const evidenceSummaries = [
  ...
  "The persistent flow tree path-copies changed paths and reuses untouched nodes by identity under its bounded policy; this structural fact does not establish latency, heap, frame, interaction, product activation, or performance readiness.",
  "The persistent line-tree boundary constructs incremental acceptance independently of an optional complete-layout QA comparison; this does not make the oracle a production execution input or establish per-keystroke performance, product activation, or performance readiness.",
  "Root V2 enforces bounded admission, source-envelope, scene, delivery, and separate work-ledger policy for its named transition rows; this does not activate later 5B-2 authority, Editor or Backend binding, browser Worker adoption, product integration, or performance readiness.",
  ...
  "The scene-delivery suite supports the named delivery repair and its current bounded behavior while earlier type-check concern and later verification retain different review ranges; this does not establish outside-range review authority, a normative implementation contract, product activation, Editor or Backend binding, browser Worker adoption, or performance readiness.",
] as const;
  ...
  it("creates the exact canonical Node, Document, and Evidence record set", async () => {
    ...
    for (const item of evidence) {
      ...
      expect(item.verificationSummary).toMatch(
        /; this (?:structural fact |scoped collision fact )?does not/iu,
      );
    }
  });
